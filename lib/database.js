...
module.exports = {
  ...
  connect: function(database, cb) {
    ...
    mongoose.connect(database, { useMongoClient: true, autoIndex: true }, function(err) {
      if (err) {
  ...
  update_masternodes_db: function(cb) {
    Masternodes.remove({}, function(){
      lib.get_masternodes(function(masternodes) {
        lib.syncLoop(masternodes.length, function(loop) {
          var i = loop.iteration();
          new Masternodes(masternodes[i]).save(function(err) {
            if (err)
              return cb(err);
            loop.next();
          });
  ...
  get_masternode: function(addr, cb) {
    Masternodes.find({ addr: addr }, function(err, masternode) {
      if (err)
        return cb([]);
      else
        return cb(masternode[0]);
    });
  },

  get_masternodes_count: function(cb) {
    Masternodes.count({ status: "ENABLED" }, function(err, count) {
      if (err)
        return cb([]);
      else
        return cb(count);
